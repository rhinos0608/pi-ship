...
import { isPlanStale, canonicalize, computeDigest, type Plan } from "./plan.js";
import type { Manifest } from "./manifest.js";
import { loadState, type LocalState } from "./state.js";
...
export async function authorizePlanApply(ctx: AuthorizationContext): Promise<void> {
  ...
  if (await isPlanStale(ctx.plan, ctx.cwd)) throw err("E_PLAN_STALE", "plan is stale; regenerate");
  if (canonicalize({ ...ctx.plan, manifest: ctx.manifest } as Plan) !== canonicalize(ctx.plan)) {
  ...
  const state = ctx.state ?? await loadState(ctx.cwd);
