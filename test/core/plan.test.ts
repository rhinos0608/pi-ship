...
import { buildPlan, computeDigest, isPlanStale } from "../../src/core/plan.js";
...
describe("buildPlan", () => {
  it("digest is deterministic", async () => {
    ...
    const p1 = await buildPlan(tmp, m, "production", opts);
    const p2 = await buildPlan(tmp, m, "production", opts);
  ...
  it("changing manifest changes digest", async () => {
    ...
    const opts = { planId: "pid-1", createdAt: "2026-01-01T00:00:00.000Z" };
    const p1 = await buildPlan(tmp, m1, "production", opts);
    const p2 = await buildPlan(tmp, { ...m1, project: "other-project" }, "production", opts);
  ...
  it("dirty worktree is flagged and included in hash", async () => {
    ...
    const p = await buildPlan(tmp, baseManifest(), "production");
  ...
  it("plan older than 30 minutes is stale", async () => {
    ...
    expect(await isPlanStale(p, tmp)).toBe(true);
  });
