...
describe("loadManifest", () => {
  ...
  it("rejects unknown top-level key", async () => {
    ...
    await assertConfigInvalid(loadManifest(tmp), "unknownKey");
  });
  ...
  it("rejects empty run.command array", async () => {
    ...
    await assertConfigInvalid(loadManifest(tmp), "run.command");
  });
  ...
  it("rejects provider other than railway", async () => {
    ...
    await assertConfigInvalid(loadManifest(tmp), "provider");
  });

  it("rejects malformed JSON", async () => {
    await writeFile(join(tmp, "pi-ship.json"), "{not json");
    await assertConfigInvalid(loadManifest(tmp), "invalid JSON");
  });
});
